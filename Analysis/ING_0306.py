...
minFreq=30.
maxFreq=250.
maxPower= None#900 # 100. # Sets the maximum for the scalogram. Varies according to the power of the signal. Change it for better visualization
colorbar=True
numBins = 2*125+1
###################################
fs = np.arange(minFreq,maxFreq,3.)
levelsWav = None#np.linspace(0., maxPower, 20, endpoint=True)
...
FScellTrace={}
SCcellTrace={}
for file in glob.glob("../0306_Test/Ant&Ca_0_Kv3_1.0_Kv7_1.0_gsinInh7.0_gsinExc0.0_*Hyper_Noise_F_data.pkl"):
    ...
    spkid = np.array(fileInfo['simData']['spkid'])
    #cells_to_mask2 = [5, 11, 12, 15, 18, 25, 29, 35, 43, 50, 55, 56, 57, 65, 67, 68, 70, 71, 72, 76, 78, 81, 93]
    cells_to_mask = [16, 18, 27, 31, 35, 50, 56, 59, 72, 73, 77, 81, 93]
    maskFS = np.isin(spkid, cells_to_mask)
