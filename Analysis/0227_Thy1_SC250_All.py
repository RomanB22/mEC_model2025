...
#### Importing all the necessary files/environments#######
...
############################################################################################
"""Here is the part to modifiy according to the simulation configuration"""
dt = 1e-2 # in ms
...
fs = np.arange(50.,350.1,3.);

chunkLength = int(np.round(thetaPeriod/dt));
...
fileID = '0227_Plots/0227_SC250_'
i = 0

for file in glob.glob("../0226_Pastoll/Const*Ant&Ca_0_Kv3_1.0_Kv7_1.0_gsinInh7.0_gsinExc3.0_*II_0.0*data.pkl"):
    print('Loading the file from specified folder')
    fileInfo = loadData(file)
    fileName = fileInfo['simConfig']['filename'][19:27]
    print(fileName)
    #j = int(fileName[])
    ...
    #print('Length of the assigned array is',np.shape(y1))
    ############################################################################################
    ...
    print(f"Peak frequency in PSD: {peak_frequency:.2f} Hz and iteration number {i}")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(fileID+fileName+"_PSD.eps", dpi=300)
    plt.savefig(fileID+fileName+"_PSD.png", dpi=300)
    plt.clf();
    ...
    plt.figure()
    Power_Index = 2e6
    plotPow(cwtPowAvg,Ncycs=1,dt=dt_rec,fs=fs,levels=np.linspace(0.,maxPowerAvg,150,endpoint=True));
    plt.ylabel("Gamma Frequency (Hz)")
    plt.xlabel("Stim Theta Phase (rad)")

    plt.tight_layout()
    plt.savefig(fileID+fileName+"_Pow_AVG.svg", bbox_inches='tight')
    plt.savefig(fileID+fileName+"_Pow_AVG.eps", bbox_inches='tight'); 
    plt.savefig(fileID+fileName+"_Pow_AVG.png", bbox_inches='tight'); 
    #plt.show()
    plt.clf();
    ...
    ax0.plot(tSim, y0, 'r-',label='Unfiltered')
