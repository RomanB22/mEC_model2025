...
NEURON {
    ...
    GLOBAL mife, mise
	RANGE sfactor, ffactor, miso, misd, mifo, mifd
}
...
PROCEDURE settables(v (mV)) { 
    ...
    tauf = ffactor*taufn/( exp( (v-taufdo)/taufdd ) + exp( -(v+taufro)/taufrd ) )
    taus = sfactor*tausn/( exp( (v-tausdo)/tausdd ) + exp( -(v+tausro)/tausrd ) )
